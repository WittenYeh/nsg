...
namespace efanna2e{
  enum Metric{
    ...
    // WittenYeh fork: temporarily replace the original AVX/SSE intrinsic
    // implementation with a byte-identical mirror of FAISS's `fvec_L2sqr`
    // (utils/distances_simd.cpp:214). FAISS uses a naive scalar loop +
    // associative-math pragma so the GCC auto-vectorizer is free to pick
    // its own reduction tree. With identical source, identical compiler,
    // and identical flags, this should emit byte-identical machine code
    // and produce bit-identical distance values to FAISS's path. Goal:
    // rule out FP-precision drift as the cause of the ~52% edge mismatch
    // observed between efanna-built and FAISS-built NSGs on the same kNN.
